...
**Created**: 2026-04-11  
**Status**: Draft  
**Input**: User description: "建立一個台灣時間每日 15:00 自動篩選台股上市股票和美股 S&P 500 成分股的網站"
...
作為一個被動的使用者，我希望系統能夠在台灣時間每天的下午 15:00 自動去抓取台股以及美股 S&P 500 成分股的收盤資料，並自動跑完篩選邏輯更新網站資料，而不需要我手動觸發。
...
2. **Given** 系統檢查發現當日為休市或無最新資料（包含 S&P 500 資料未更新），**When** 排程執行時，**Then** 系統會暫停後續篩選動作，並在網站上註明「資料未更新」或「休市」。
...
- **FR-002**: 系統的抓取範圍必須包含「台股上市股票」與「美股 S&P 500 成分股」。
- **FR-003**: 系統只允許抓取並處理「收盤後」的資料，不處理盤中即時資料。
- **FR-004**: 系統必須實作以下的「老余三問」股票篩選核心邏輯，每次篩選須確保取得最近 200 根日 K 以及 200 根週 K 資料作爲分析基礎：
  - **位置**: 判斷週線處於下邊界（當前價格落在過去 200 週價格區間的底部 10% 以內）。
  - **慣性**: 判斷週線 K 棒出現長下影線（下影線長度大於實體與上影線總和，代表價格下不去）。
  - **圖**: 判斷日線 K 棒走勢符合「破底翻」型態（定義為呈現 P0 - P5 波動，且 P5 與 P1 的價格落差需在 3% 以內，且 P5 距離 P3 的價格空間修改為 (P2 - P3) * 0.25 <= (P5 - P3) <= (P2 - P3) * 0.75，確保明確的築底特徵）。
...
- **FR-009**: 當資料來源 API 抓取失敗或連線逾時，系統必須最多進行 3 次重試機制（重試粒度為「單檔股票重試」，而非整個批次重新抓取）。
- **FR-010**: 網站必須具備追蹤與顯示「資料更新狀態」的功能。考量到抓取時間可能橫跨兩小時以上，網站上必須針對「台股」與「美股（S&P 500）」的狀態「分開獨立顯示」（如：抓取中、已抓取完成、更新失敗、休市）。
- **FR-011**: 系統必須自動從外部 API 動態獲取最新的台股上市股票與美股 S&P 500 成分股清單，並套用價量前置過濾條件（台股：當日成交量 >= 1000 張；美股：股價 >= 10 USD 且當日成交量 >= 1,000,000 股），以優化 API 抓取效能並確保涵蓋最新上市或下市的變動。
- **FR-012**: 系統的爬蟲腳本必須支援透過 `TARGET_DATE` 環境變數來指定「虛擬執行日期」，以利開發者在週末休市期間進行手動驗證或未來歷史回測。
...
- **StockAsset**: 記錄單一股票的代號、名稱、市場別（台股/美股）與 TradingView URL 的基礎資訊。
